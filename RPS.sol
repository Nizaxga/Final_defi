...
contract RPS {
    ...
    constructor(address _commitReveal, address _convert, address _timeUnit, address _token) {
    ...
    function addPlayer() public payable {
        ...
        require(token.allowance(msg.sender, address(this)) > 0.000001 ether, "allowance please");
    ...
    function commit(uint choice) public  {
        ...
        numInput++;
        if (numInput == 2) {
            token.transferFrom(players[0], address(this), 0.000001 ether);
            token.transferFrom(players[1], address(this), 0.000001 ether);
            reward += 0.000002 ether;
        }
    }
    ...
    function refund() public {
        ...
        } else if (numPlayer == 2 && numInput == 2 && timeUnit.elapsedSeconds() >= TimeOut) {
